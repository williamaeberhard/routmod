# routnll blockwise no lake: neg log lik for routing | v0.9.1
# * Change log:
#    - v0.9.1: changed dgamma to user-defined gammakern, avoiding computation
#              of gamma pdf normalization factor
#    - v0.9: initial version, forked from routnll_blockwise v0.9. Functionally
#      equivalent to routnll_blockwise v0.8 (no conditioning on lake dummy).
...
rout_nll_block_nolake_ini <- function(par){
	# * par vector order:
	#   - log_wscale (1)
...
rout_nll_block_nolake <- function(par){
	# * par vector order:
	#   - log_wscale (1)
	#   - wshapebeta (p, now incl intercept)
